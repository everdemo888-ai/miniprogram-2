...
以下用例模版位于 `test/requestVideoJob.json`，替换 `videoLink` 后按 TC-001 步骤执行即可：

| 编号 | 用例名称 | 关键验证点 |
|:--:|------|------|
| TC-006 | 空链接 | `success: false`, 错误提示包含"链接为空" |
| TC-007 | 不支持的平台 | `success: false`, 错误提示包含"暂不支持" |
| TC-008 | 重复提交同一链接 | 返回新 `jobId`，`status: "queued"` |
